...
const Emails = () => {
  ...
  const handleBulkSend = async () => {
    ...
    // Chunk into small batches so each HTTP request finishes well under
    // timeouts (each email takes ~3-7s incl. the 2s backend gap).
    // Chunks are sent SEQUENTIALLY to avoid Gmail rate limits, OAuth token
    // races, and DB connection pool exhaustion. If a chunk request fails,
    // we RECONCILE actual statuses from the DB (the backend often finishes
    // sending even when the HTTP call times out) instead of blindly
    // counting the whole chunk as failed.
    const CHUNK_SIZE = 5;
    let sentCount = 0;
    let failedCount = 0;
    ...
    for (let i = 0; i < idsToSend.length; i += CHUNK_SIZE) {
      ...
      setBulkSendProgress(prev => ({ ...prev, current: `Sending batch ${chunkNum}/${totalChunks} (${i + 1}-${Math.min(i + CHUNK_SIZE, idsToSend.length)} of ${idsToSend.length})...` }));
      ...
      try {
        ...
        console.error(`Batch ${chunkNum}/${totalChunks} request failed:`, chunkErr);
        showNotification('error', `Batch ${chunkNum}/${totalChunks} network issue (${chunkErr?.response?.data?.detail || chunkErr.message}). Verifying actual results...`);
        // The backend may have completed these sends despite the request
        // failing. Reconcile with real DB statuses before counting.
        let reconciled = false;
        for (const delay of [3000, 8000]) {
          await new Promise(r => setTimeout(r, delay));
          try {
            const st = await api.post('/api/leads/status-batch', { lead_ids: chunk }, { timeout: 15000 });
            const statusMap = {};
            (st.data.statuses || []).forEach(s => { statusMap[String(s.id)] = s.email_status; });
            let allResolved = true;
            let actuallySent = 0;
            let actuallyFailed = 0;
            for (const id of chunk) {
              const s = statusMap[String(id)];
              if (s === 'SENT') actuallySent++;
              else if (s === 'FAILED') actuallyFailed++;
              else allResolved = false; // still processing or unknown — retry after next delay
            }
            if (allResolved || delay === 8000) {
              const unresolved = chunk.length - actuallySent - actuallyFailed;
              sentCount += actuallySent;
              failedCount += actuallyFailed + unresolved;
              reconciled = true;
              break;
            }
          } catch { /* keep waiting */ }
      ...
      {showTemplatePicker && (
        <div className="fixed inset-0 z-[4000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="w-full max-w-xl bg-[#0d1117] border border-blue-500/20 rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-white tracking-tight">
                    {templatePickerStep === 0 ? 'Choose Draft Template' : 'Choose Signature'}
                  </h3>
            ...
            {/* Step Indicator */}
            <div className="px-8 pt-6 flex items-center gap-2 shrink-0">
              <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${templatePickerStep === 0 ? 'bg-blue-500/15 text-blue-400 border border-blue-500/20' : 'bg-white/5 text-slate-500 border border-white/5'}`}>
                {templatePickerStep === 0 ? <div className="w-1.5 h-1.5 rounded-full bg-blue-500" /> : <Check className="w-3 h-3 text-emerald-400" />}
                1/2 · Draft Template
              </div>
              <div className="w-5 h-px bg-white/10" />
              <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${templatePickerStep === 1 ? 'bg-amber-500/15 text-amber-400 border border-amber-500/20' : 'bg-white/5 text-slate-500 border border-white/5'}`}>
                {templatePickerStep === 1 && <div className="w-1.5 h-1.5 rounded-full bg-amber-500" />}
                2/2 · Signature
              </div>
            </div>

            {/* ── STEP 1/2: Templates ── */}
            {templatePickerStep === 0 && (
              <>
                ...
                <div className="p-8 bg-black/20 border-t border-white/5 flex gap-4">
                  ...
                  <button 
                    onClick={() => setTemplatePickerStep(1)}
            ...
            {templatePickerStep === 1 && (
              <>
                ...
                <div className="p-8 bg-black/20 border-t border-white/5 flex gap-4">
                  <button 
                    onClick={() => setTemplatePickerStep(0)}
                    className="flex items-center justify-center gap-2 py-4 px-6 bg-white/5 hover:bg-white/10 text-slate-300 text-[11px] font-black uppercase tracking-widest rounded-[20px] transition-all"
                  >
                    <ChevronLeft className="w-4 h-4" /> Back
                  </button>
                  <button 
                    onClick={handleGenerateWithTemplate}
                    className="flex-[2] py-4 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white text-[11px] font-black uppercase tracking-widest rounded-[20px] transition-all shadow-xl shadow-amber-500/20 flex items-center justify-center gap-3"
                  >
