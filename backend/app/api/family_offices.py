...
def list_offices(search: str | None = None, user_id: str | None = Header(None, alias="X-User-Id")):
    return get_all_family_offices(search, user_id)

@router.get("/family-offices/{office_id}")
def get_office(office_id: int, user_id: str | None = Header(None, alias="X-User-Id")):
...
def update_office(office_id: int, req: FamilyOfficeUpdate, user_id: str | None = Header(None, alias="X-User-Id")):
...
def list_office_leads(office_id: int, user_id: str | None = Header(None, alias="X-User-Id")):
    office = get_office(office_id, user_id)
    return get_office_leads(office['name'], user_id)

@router.post("/family-offices/sync")
def sync_offices(user_id: str | None = Header(None, alias="X-User-Id")):
...
def search_rocketreach(office_id: int, req: RocketReachSearch, user_id: str | None = Header(None, alias="X-User-Id")):
    office = get_office(office_id, user_id)
...
def bulk_sync(user_id: str | None = Header(None, alias="X-User-Id")):
