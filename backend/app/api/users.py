...
@router.get("/users/pilot-settings", response_model=JsonObject)
def get_pilot_settings(user_id: str | None = Header(None, alias="X-User-Id")):
...
def update_pilot_settings(req: PilotSettingsRequest, user_id: str | None = Header(None, alias="X-User-Id")):
...
def create_user(user: UserCreate):
    ...
    # Hash password with bcrypt (salted), matching login verification
...
def get_user_productivity(user_id: str | None = Header(None, alias="X-User-Id")):
...
def get_active_users(user_id: str | None = Header(None, alias="X-User-Id")):
...
def trigger_admin_report(req: ReportRequest, user_id: str | None = Header(None, alias="X-User-Id")):
    ...
    try:
        ...
        if leads:
            ...
            for row_num, l in enumerate(leads, 1):
                ...
                status = 'NOT STARTED' if raw_status in ('', 'PENDING') else raw_status
        ...
        try:
            ...
            logger.exception(f"Report Dispatch Error: {str(e)}")
...
def get_my_history(user_id: str | None = Header(None, alias="X-User-Id")):
