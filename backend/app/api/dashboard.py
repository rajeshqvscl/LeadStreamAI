...
def get_card_detail(
    ...
    if card_type == 'ingested':
        ...
        # Opens/events are filtered on updated_at (when the open was recorded),
        # matching /dashboard/stats — not lead import time.
        open_cond = "(email_status = 'OPENED')"
        event_date = date_cond.replace("created_at", "updated_at") if date_cond else ""
        cur.execute(f"SELECT COUNT(*) FROM leads_raw WHERE {user_cond} AND {open_cond} {event_date}", user_params + date_params)
        total = cur.fetchone()[0] or 0
        data_sql = f"""
            SELECT id, first_name, last_name, email, company_name, persona,
                   email_status, created_at, source
            FROM leads_raw WHERE {user_cond} AND {open_cond} {event_date}
        ...
        event_date = date_cond.replace("created_at", "updated_at") if date_cond else ""
        cur.execute(f"SELECT COUNT(*) FROM leads_raw WHERE {user_cond} AND {click_cond} {event_date}", user_params + date_params)
        total = cur.fetchone()[0] or 0
        data_sql = f"""
            SELECT id, first_name, last_name, email, company_name, persona,
                   email_status, created_at, source
            FROM leads_raw WHERE {user_cond} AND {click_cond} {event_date}
            ORDER BY updated_at DESC LIMIT %s OFFSET %s
        """
        cur.execute(data_sql, user_params + date_params + [per_page, offset])
