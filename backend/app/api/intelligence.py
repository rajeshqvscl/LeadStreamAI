...
def _is_admin(user_id: str | None, uid: str | None, conn=None) -> bool:
    """Admin = ADMIN role in users table OR numeric user id 1 (the admin account)."""
    if str(uid) == "1":
        return True
    if not user_id:
        return False
    ...
    try:
        ...
        try:
            cur.execute("SELECT role FROM users WHERE id = %s", (uid_val,))
            row = cur.fetchone()
        finally:
            cur.close()
    finally:
...
async def auto_enrich_sectors(user_id: str | None = Header(None, alias="X-User-Id")):
    """
    ...
    try:
        ...
        logger.exception("auto_enrich_error", error=str(e))
...
async def ai_deep_classify(user_id: str | None = Header(None, alias="X-User-Id")):
    """
    ...
    try:
        ...
        for lead in leads:
            ...
            try:
                ...
                logger.exception("ai_classify_error", lead_id=lead['id'], error=str(e))
                continue

        conn.commit()
        return {"success": True, "updated": updated}
    except Exception as e:
        logger.exception("deep_classify_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if 'conn' in locals():
            conn.close()
...
"""
        ...
        logger.exception("intelligence_chat_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/analyze-lead/{lead_id}")
async def analyze_lead_manually(lead_id: int, user_id: str | None = Header(None, alias="X-User-Id")):
    """
    ...
    try:
        ...
        if pitch_deck_url and pitch_deck_url.startswith("http"):
            try:
                ...
                if not file_data:
                    ...
                    from app.core.http_client import get
                    res = get(actual_url, timeout=30)
        ...
        try:
            ...
            logger.exception(f"STRICT RAG ERROR: {rag_err}")
            rag_intel = {
        ...
        try:
            ...
            logger.exception(f"Agent Service Error: {agent_err}")
        ...
        logger.exception("analyze_lead_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if 'conn' in locals():
            conn.close()
...
async def compare_leads(payload: CompareLeadsRequest, user_id: str | None = Header(None, alias="X-User-Id")):
    """
    ...
    try:
        ...
        if is_admin:
            cur.execute("SELECT id, company_name, sector, rag_advice, rag_intelligence FROM leads_raw WHERE id = ANY(%s)", (payload.lead_ids,))
        else:
            cur.execute("SELECT id, company_name, sector, rag_advice, rag_intelligence FROM leads_raw WHERE id = ANY(%s) AND user_id = %s", (payload.lead_ids, uid))
        leads = cur.fetchall()
        ...
        logger.exception("compare_leads_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if 'conn' in locals():
            conn.close()
...
async def get_rag_debug_stats(user_id: str | None = Header(None, alias="X-User-Id")):
    """
    ...
    try:
        ...
        logger.exception("rag_debug_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if 'conn' in locals():
            conn.close()
...
async def get_lead_ai_timeline(lead_id: int, user_id: str | None = Header(None, alias="X-User-Id")):
    """
    ...
    try:
        ...
        try:
            ...
            logger.exception("timeline_ai_error", error=str(e))
        ...
        logger.exception("get_timeline_error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if 'conn' in locals():
            conn.close()
