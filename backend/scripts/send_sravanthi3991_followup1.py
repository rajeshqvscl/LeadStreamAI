"""
Send follow-up STAGE 1 to Sravanthi (lead id 3991, sravanthim674@gmail.com, user 3 / Kajal).
Single-lead, production-mirroring path. Sends the 1st follow-up (followup_1) of the
lead's own draft template (kajal_mam_Strategic Advisory Support for Portfolio Companies)
into the existing Gmail thread.

Run:
  python scripts/send_sravanthi3991_followup1.py --dry-run   # preview only, NO send
  python scripts/send_sravanthi3991_followup1.py            # real Gmail send
"""
import sys
import os
import re
...
def main():
    ...
    # 3. Subject: Re: the actual last sent email subject (thread coherence)
    orig_subject = lead.get('last_outreach_subject') or lead.get('first_outreach_subject') or ''
    ...
    if DRY_RUN:
        print("[DRY-RUN] No DB changes, no send. Exiting.")
        cur.close()
        conn.close()
        return
    ...
    if success:
        ...
        conn.commit()
        add_activity_log(LEAD_ID, "FOLLOWUP_APPROVED", "Stage 1 follow-up sent to Sravanthi (manual, lead 3991)", "user", USER_ID)
        print("DONE: follow-up 1 sent to Sravanthi (lead 3991).")
    else:
