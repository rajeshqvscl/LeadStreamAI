...
def update_templates():
    ...
    # Update Yashika's template
    cur.execute(
        "UPDATE prompts SET content = %s WHERE name = 'yashika_draft_agritech'",
        (AGRITECH_CONTENT,)
    )
    
    # Also fix Palak Mam's template signature if it was hardcoded
    # (Assuming it might need Sender placeholders now)
